<template>
  <q-card class='container'>
    <q-card-section>
      <q-item-label>{{ $t('MSG_CREATE_DISCOUNT_POOL') }}</q-item-label>
    </q-card-section>
    <q-item-section>
      <q-item>
        <q-item-label>{{ $t('MSG_APP_NAME') }}: {{ selectedApp?.App.Name }}</q-item-label>
      </q-item>
      <q-input
        v-model='myName'
        :label='$t("MSG_NAME")'
      >
        <template #prepend>
          <q-icon name='window' />
        </template>
      </q-input>
      <q-input
        v-model='startTime'
        :label='$t("MSG_START_TIME")'
      >
        <template #prepend>
          <q-icon name='window' />
        </template>
      </q-input>
      <q-input
        v-model='durationDays'
        :label='$t("MSG_DURATION_DAYS")'
      >
        <template #prepend>
          <q-icon name='window' />
        </template>
      </q-input>
      <q-input
        v-model='discount'
        :label='$t("MSG_DISCOUNT")'
      >
        <template #prepend>
          <q-icon name='window' />
        </template>
      </q-input>
      <q-input
        v-model='message'
        :label='$t("MSG_MESSAGE")'
      >
        <template #prepend>
          <q-icon name='window' />
        </template>
      </q-input>
    </q-item-section>
    <q-item-section>
      <q-btn
        class='submit-btn'
        :label='$t("MSG_SUBMIT")'
        @click='onSubmit'
      />
    </q-item-section>
  </q-card>
</template>

<script setup lang='ts'>
import { defineProps, toRef, computed, defineEmits, watch, ref } from 'vue'
import { Application } from 'src/store/applications/types'
import { DiscountPool } from 'src/store/inspire/types'
import { useStore } from 'src/store'

interface Props {
  selectedApp?: Application
  editDiscountPool?: DiscountPool
}

const props = defineProps<Props>()

const selectedApp = toRef(props, 'selectedApp')
const editDiscountPool = toRef(props, 'editDiscountPool')

const editDiscount = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.Discount : 10
})
const discount = ref(editDiscount.value)

const editStart = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.Start : (new Date().getTime() / 1000 + 24 * 60 * 60).toFixed(0)
})
const startTime = ref(editStart.value)

const editDurationDays = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.DurationDays : 60
})
const durationDays = ref(editDurationDays.value)

const editMessage = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.Message : ''
})
const message = ref(editMessage.value)

const editName = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.Name : ''
})
const myName = ref(editName.value)

const editDiscountPoolID = computed(() => {
  return editDiscountPool.value ? editDiscountPool.value.ID : undefined
})
const id = ref(editDiscountPoolID.value)

const store = useStore()
const userID = computed(() => store.getters.getLoginedUser.User?.ID)

const discountPool = computed(() => {
  return {
    ID: id.value as string,
    AppID: selectedApp.value?.App.ID,
    ReleaseByUserID: userID.value,
    Name: myName.value,
    Message: message.value,
    Start: startTime.value,
    DurationDays: durationDays.value,
    Discount: discount.value
  } as DiscountPool
})

watch(discountPool, () => {
  emit('update', discountPool.value)
})

const emit = defineEmits<{(e: 'submit', info: DiscountPool): void,
  (e: 'update', info: DiscountPool): void
}>()

const onSubmit = () => {
  emit('submit', discountPool.value)
}

</script>

<style lang='sass' scoped>
.container
  padding: 24px
  min-width: 800px

.submit-btn
  margin-top: 24px
  background-color: $blue-6
  color: white
</style>
